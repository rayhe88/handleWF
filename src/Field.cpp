//////////////////////////////////////////////////////////////////////
// Task one: Replace instances of the Rvector with STL std::vector
//////////////////////////////////////////////////////////////////////
...
// removed Rvector with std::vector
double Field::gaussiana(std::vector<double> r, int mu) {
    int lx = wf.vang[3 * mu];
    ...
    for (int i = 0; i < npoints_x; i++) {
        ...
        for (int j = 0; j < npoints_y; j++) {
            ...
            for (int k = 0; k < npoints_z; k++) {
                ...
                double den = DensitySYCL2(
                    wf.norb, wf.npri, wf.icntrs.data(), wf.vang.data(), r, coor,
                    wf.depris.data(), wf.dnoccs.data(), wf.dcoefs.data());
...
void Field::evalDensity_sycl() {
        ...
        q.submit([&](sycl::handler &h) {
            ...
            h.parallel_for<class Field2>(
                sycl::range<1>(nsize), [=](sycl::id<1> idx) {
                    ...
                    const int *icnt_ptr =
                        icnt_acc.get_multi_ptr<sycl::access::decorated::no>()
                            .get_raw();
                    const int *vang_ptr =
                        vang_acc.get_multi_ptr<sycl::access::decorated::no>()
                            .get_raw();
                    const double *coor_ptr =
                        coor_acc.get_multi_ptr<sycl::access::decorated::no>()
                            .get_raw();
                    const double *eprim_ptr =
                        eprim_acc.get_multi_ptr<sycl::access::decorated::no>()
                            .get_raw();
                    const double *nocc_ptr =
                        nocc_acc.get_multi_ptr<sycl::access::decorated::no>()
                            .get_raw();
                    const double *coef_ptr =
                        coef_acc.get_multi_ptr<sycl::access::decorated::no>()
                            .get_raw();

                    field_acc[idx] =
                        DensitySYCL2(norb, npri, icnt_ptr, vang_ptr, cart,
    ...
    for (int i = 0; i < nsize; i++) field.push_back(field_local[i]);
